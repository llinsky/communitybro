...
Val* BroFunc::Call(val_list* args, Frame* parent) const
	...
	//this seems wasteful in any case
...
Val* BroFunc::LuaCall(val_list* args) const
{
#ifdef PROFILE_BRO_FUNCTIONS
	DEBUG_MSG("Function: %s\n", Name());
...
#ifdef LUA_DEBUG
	reporter->Info("Starting BroFunc: %s", Name());
#endif

	//TODO: Validate memory management -- I may need to delete this frame
...
#ifdef LUA_DEBUG
	reporter->Info("Successfully called BroFunc: %s, returning result", Name());
#endif
...
Val* BuiltinFunc::LuaCall(val_list* args) const
{
#ifdef PROFILE_BRO_FUNCTIONS
	DEBUG_MSG("Function: %s\n", Name());
	...
	else if ( result && result->Type()->Tag() != yt->Tag() && yt->Tag() != TYPE_ANY)
		{
		reporter->InternalError("plugin returned wrong type (got %d, expecting %d) for %s",
					result->Type()->Tag(), yt->Tag(), this->Name());
		}
#endif
	...
	//reporter->Info("\tBuiltin Function called: %s\n", d.Description());
#endif
