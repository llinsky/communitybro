...
#ifdef DEBUG
	//Leo edit: this event seems wasteful during normal operation, so I'm limiting to debug
	if ( new_event )
		NewEvent(vl);
#endif

#ifdef ENABLE_LUAJIT
	//TODO: Incorporate this into the event framework so it's only called as an Event during a Drain().
	lua_mgr->LuajitTryEvent(Name(), vl);
#endif
