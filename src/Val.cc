...
Val* Val::Unserialize(UnserialInfo* info, TypeTag type, const BroType* exact_type)
	...
	if ( type != TYPE_ANY && (v->Type()->Tag() != type \
		|| (exact_type && ! same_type(exact_type, v->Type()))) )
		{
			...
			char s2[100];
			char s3[100];
			TypeTagToString(type, s1); TypeTagToString(v->Type()->Tag(), s2); TypeTagToString(exact_type->Tag(), s3);
			reporter->Info("Type: %s  v type: %s  exact_type: %s", s1, s2, s3);
#endif
...
const char * Val::CoerceToCString() const
{
	if ( type->InternalType() == TYPE_INTERNAL_STRING )
	{
		return reinterpret_cast<const char*>( val.string_val->Bytes() );
	}
	return NULL;
}
#endif
...
#ifdef ENABLE_LUAJIT
void PatternVal::AsLuaPattern(ODesc* d) const
	{ ValDescribe(d); }
int PatternVal::MatchExactly(const char* s)
	{ return val.re_val->MatchExactly(s); }
int PatternVal::MatchAnywhere(const char* s)
	{ return val.re_val->MatchAnywhere(s); }
int PatternVal::MatchPrefix(const char* s)
	{ return val.re_val->MatchPrefix(s); }
#endif
...
Val* RecordVal::Lookup(const char* field, bool with_default, bool lua_override ) const
	{
