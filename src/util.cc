...
FILE* rotate_file(const char* name, RecordVal* rotate_info)
	...
	if ( link(name, newname) < 0 || stat(newname, &dummy) < 0 )
		...
		fclose(newf);
		if(unlink(newname) || unlink(tmpname))
		{
			reporter->Error("%s: Unable to unlink %s or %s", __FUNCTION__, newname, tmpname);
		}
		return 0;
		}
...
double check_shannon_entropy(unsigned int len, const char* data)
{
	float entropy = 0;
	float count;
	unsigned int i;
	char byte_counts[256];
	memset(byte_counts, 0, sizeof(byte_counts));
	for (i = 0; i < len; i++)
	{
		byte_counts[(uint8_t)data[i]]++;
	}

	for (i = 0; i < sizeof(byte_counts); i++)
	{
		if ((uint8_t)byte_counts[i] > 0)
		{
			count = (float) byte_counts[i] / (float) len;
...
double check_printable_ascii(unsigned int len, const char* data)
{
	double printable_count = 0;
	unsigned int index = 0;
	while (index < len)
	{
		if ( ((uint8_t)data[index] > 31) && ((uint8_t)data[index] < 127)) {
			printable_count++;
		}
		index++;
	}

	double printable_percentage = 100*printable_count/((double)len);
	return printable_percentage;
}
