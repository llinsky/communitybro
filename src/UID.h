...
extern uint32 g_rolling_conn_couter;
...
public:
	...
	//void Set_conn_uid(void);
	void Set_conn_uid(double start_time, const u_char *peer_description);
	...
	/*
	 * Returns a to_hex representation
	 * @param prefix An optional string prefix.
	 * @return the string representing the UID
	 */
	std::string conn_uid_to_string(std::string prefix = "") const;
...
private:
	...
	std::pair<uint32,uint32> Determine_Worker_and_Process_IDs(const u_char *peer_description);
};

inline UID::UID(const UID& other)
	{
	memcpy(uid, other.uid, sizeof(uid));
	conn_uid = other.conn_uid;
...
inline std::string UID::conn_uid_to_string(std::string prefix) const
{
	if ( ! initialized ) {
		reporter->InternalError("use of uninitialized UID");
	}

	return prefix + conn_uid;
}
