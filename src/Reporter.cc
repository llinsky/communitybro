...
#define TO_STDERR(x) ((x) || BifConst::Reporter::all_to_stderr ? stderr : 0)
...
Reporter::Reporter()
{
	...
	openlog((const char *)log_ident, 0, log_facility);
	}
...
void Reporter::Info(const char *fmt, ...)
{
	va_list ap;
	va_start(ap, fmt);
	FILE *out = TO_STDERR(info_to_stderr);
...
void Reporter::Warning(const char *fmt, ...)
{
	va_list ap;
	va_start(ap, fmt);
	FILE *out = TO_STDERR(warnings_to_stderr);
...
void Reporter::Error(const char *fmt, ...)
{
	...
	va_list ap;
	va_start(ap, fmt);
	FILE *out = TO_STDERR(errors_to_stderr);
...
void Reporter::AnalyzerError(analyzer::Analyzer *a, const char *fmt, ...)
{
	...
	DoLog("analyzer error", reporter_error, stderr, 0, 0, true, true, 0, fmt, ap);
	va_end(ap);
}
...
void Reporter::InternalWarning(const char *fmt, ...)
{
	...
	// TODO: would be nice to also log a call stack.
	DoLog("internal warning", reporter_warning, out, 0, 0, true, true, 0, fmt, ap);
	va_end(ap);
}

void Reporter::Syslog(const char *fmt, ...)
{
	if (unlikely(reading_traces)) {
		return;
	}

	va_list ap;
	va_start(ap, fmt);
	vsyslog(log_priority, fmt, ap);
	va_end(ap);
}
					 ...
					 const char *postfix, const char *fmt, va_list ap)
{
	...
	if (via_events && !in_error_handler) {
		if (locations.size())	{
			...
			raise_event = PLUGIN_HOOK_WITH_RESULT(HOOK_REPORTER,
												  HookReporter(prefix, event, conn, addl, location,
															   nullptr, nullptr, time, buffer),
												  true);
		}
	}

	if (unlikely(raise_event && event && via_events && !in_error_handler)) {
	...
	if (likely(out || reporter_to_syslog)) {
		string s = "";
		...
		if (unlikely(reporter_to_syslog && !reading_traces)) {
			syslog(log_priority, "%s", s.c_str());
		}
	}
