...
#ifdef ENABLE_LUAJIT
	const char * CoerceToCString() const;
	void LuaRefReset(int count) { RefCntOverride(count); }
#endif
...
#define CONST_ACCESSOR2(tag, ctype, accessor, name) \
...
#ifdef ENABLE_LUAJIT
	bro_int_t AsInt() const
	{
		return (bro_int_t) AsNumber();
	}
	bro_uint_t AsCount() const
	{
		return (bro_uint_t) AsNumber();
	}
	double AsDouble() const
	{
		return AsNumber();
	}
	//Customizing accessors for numbers
	double AsNumber() const
	{
		...
		else if (type->Tag() == TYPE_COUNT)
		{
			return val.uint_val;
		}
		else if (type->Tag() == TYPE_DOUBLE)
		{
			return val.double_val;
		}
		else
		{
			CHECK_TAG(type->Tag(), TYPE_INT, "Val::CONST_ACCESSOR", type_name);
			return val.int_val; //this will fail
		}
	}
#endif
...
#ifdef ENABLE_LUAJIT
	//used for time (todo: and intervals?)
	double ForceAsDouble() const		{ return val.double_val; }
#endif
...
#ifdef ENABLE_LUAJIT
	void AsLuaPattern(ODesc *d) const;
...
#ifdef ENABLE_LUAJIT
	TableType *GetTableType() { return table_type; }
#endif
...
public:
	 ...
	 */
	Val* Lookup(const char* field, bool with_default = false, bool lua_override = false) const;
...
#ifdef ENABLE_LUAJIT
	RecordType *LuaRecordType() { return record_type; }

	val_list *LuaRecordVals() { return val.val_list_val; }
#endif
...
public:
	...
	const char * ToString() const
		{ return type->AsEnumType()->Lookup(val.int_val); }
	int ToNumber() const
		{ return val.int_val; }
#endif
...
#ifdef ENABLE_LUAJIT
	VectorType* lua_vector_type() { return vector_type; }
#endif
