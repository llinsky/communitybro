...
#ifdef ENABLE_LUAJIT
LuajitManager *lua_mgr = 0;
BifMeta *bifmeta_mgr = 0;
#endif
...
void usage()
	{
	fprintf(stderr, "communityBro version %s\n", bro_version());
...
#ifdef ENABLE_LUAJIT
	fprintf(stderr, "    -l|--lua <luascriptdir>     	| enable luajit, and load lua scripts from directory <luascriptdir>\n");
#endif
...
int main(int argc, char** argv)
...
#ifdef ENABLE_LUAJIT
	char lua_script_dir[100] = { '\0'};
	strcpy(lua_script_dir, DEFAULT_LUA_SCRIPT_DIR);
#endif
	...
	while ( (op = getopt_long(argc, argv, opts, long_opts, &long_optsind)) != EOF )
		...
		case 'v':
			fprintf(stdout, "communityBro version %s\n", bro_version());
			exit(0);
			break;
	...
	/* //no longer needed
	bifmeta_mgr = new BifMeta();
	if (unlikely(!bifmeta_mgr)) {
		reporter->FatalError("Unable to load Bif metadata");
	}*/
	
	lua_mgr = new LuajitManager(lua_script_dir);
	if (likely(lua_mgr))
		lua_loaded = lua_mgr->Load();	
#endif
	...
	reporter->ReportViaEvents(BifConst::Reporter::report_via_events);
