...
uint32 g_rolling_conn_counter = 0;
...
RecordVal* Connection::BuildConnVal()
	...
	if ( ! conn_val )
		...
		if ( ! uid ) {
		   const u_char *t_peer = NULL;

		   uid.Set(bits_per_uid);

		   ID *descr = global_scope()->Lookup("peer_description");
		   if(descr) {
			   StringVal *sv = (StringVal *)descr->ID_Val();
			   t_peer = sv->Bytes();
		   }

		   uid.Set_conn_uid(start_time, t_peer);
		}

		conn_val->Assign(7, new StringVal(uid.conn_uid_to_string("").c_str()));
