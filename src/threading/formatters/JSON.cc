...
bool JSON::Describe(ODesc* desc, Value* val, const string& name) const
	...
	switch ( val->type )
		...
		case TYPE_COUNTER:
			{
			// JSON doesn't support unsigned 64bit ints, so convert it to 
			//   "id": "10765432100123456789", <-- masked to 53 bits
			desc->Add(val->val.uint_val & JSON_MAX_SAFE_INT_MASK);
			break;
			}
		...
		case TYPE_TIME:
			...
			if ( timestamps == TS_ISO8601 )
				...
				// JSON doesn't support unsigned 64bit ints, so convert it to 
				//   "id": "10765432100123456789", <-- masked to 53 bits
				desc->Add(ts & JSON_MAX_SAFE_INT_MASK);
				}
