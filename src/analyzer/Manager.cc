...
void Manager::InitPostScript()
	{
		AddCommonAnalyzer(GetComponentTag("BACKDOOR"));
		AddCommonAnalyzer(GetComponentTag("CONNSIZE"));
		AddCommonAnalyzer(GetComponentTag("INTERCONN"));
		AddCommonAnalyzer(GetComponentTag("STEPPINGSTONE"));
		AddCommonAnalyzer(GetComponentTag("TCPSTATS"));
		AddCommonAnalyzer(GetComponentTag("TEREDO"));
		AddCommonAnalyzer(GetComponentTag("PIA_UDP"));
		AddCommonAnalyzer(GetComponentTag("PIA_TCP"));
		AddCommonAnalyzer(GetComponentTag("PIA"));
	}
...
bool Manager::IsCommonAnalyzer(Tag analyzer)
{
	if (common_analyzers[analyzer.Type()] == (analyzer.Subtype() + 999))
	{
		return true;
	}	
	return false;
...
void Manager::AddCommonAnalyzer(Tag analyzer)
{
	common_analyzers[analyzer.Type()] = analyzer.Subtype() + 999;
}
