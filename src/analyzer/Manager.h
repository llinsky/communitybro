...
public:
	...
	/*
	* Tests whether a given analyzer tag should be ignored for purposes of
	* counting how many analyzers attached to a particular connection 
	*/
	bool IsCommonAnalyzer(Tag analyzer);

	/*
	* Adds a common analyzer.
	*/
	void AddCommonAnalyzer(Tag analyzer);

private:
	typedef set<Tag> tag_set;
	typedef map<uint32, tag_set*> analyzer_map_by_port;

	map<uint32, uint32> common_analyzers;
