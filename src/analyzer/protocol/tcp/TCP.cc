...
static RecordVal* build_psh_packet_val(int is_orig, const IP_Hdr* ip,
                                       const struct tcphdr* tcp) {
	int winscale = -1;
	int MSS = 0;
	int SACK = 0;
	...
	RecordVal* v = new RecordVal(PSH_packet);
	...
	v->Assign(6, new Val(MSS, TYPE_COUNT));
	v->Assign(7, new Val(SACK, TYPE_BOOL));
	v->Assign(8, new Val(int(tcp->th_flags & TH_FIN), TYPE_BOOL));

	return v;
}

static RecordVal* build_urg_packet_val(int is_orig, const IP_Hdr* ip,
                                       const struct tcphdr* tcp) {
	int winscale = -1;
	int MSS = 0;
	int SACK = 0;
	...
	RecordVal* v = new RecordVal(URG_packet);
...
static RecordVal* build_cwr_packet_val(int is_orig, const IP_Hdr* ip,
                                       const struct tcphdr* tcp) {
	int winscale = -1;
	int MSS = 0;
	int SACK = 0;
	...
	RecordVal* v = new RecordVal(CWR_packet);
...
static RecordVal* build_ece_packet_val(int is_orig, const IP_Hdr* ip,
                                       const struct tcphdr* tcp) {
	int winscale = -1;
	int MSS = 0;
	int SACK = 0;
	...
	RecordVal* v = new RecordVal(ECE_packet);
...
void TCP_Analyzer::DeliverPacket(int len, const u_char* data, bool is_orig,
	...
	if ( unlikely(flags.PUSH()) ) {
			if ( connection_PSH_packet ) {
				RecordVal* PSH_vals = build_psh_packet_val(is_orig, ip, tp);

				val_list* vl = new val_list;
				vl->append(BuildConnVal());
				vl->append(PSH_vals->Ref());
				ConnectionEvent(connection_PSH_packet, vl);

				Unref(PSH_vals);
			}
		}

	if ( unlikely(flags.URG()) ) {
			if ( connection_URG_packet ) {
				RecordVal* URG_vals = build_urg_packet_val(is_orig, ip, tp);

				val_list* vl = new val_list;
				vl->append(BuildConnVal());
				vl->append(URG_vals->Ref());
				ConnectionEvent(connection_URG_packet, vl);

				Unref(URG_vals);
			}
		}

	if ( unlikely(flags.CWR()) ) {
			if ( connection_CWR_packet ) {
				RecordVal* CWR_vals = build_cwr_packet_val(is_orig, ip, tp);

				val_list* vl = new val_list;
				vl->append(BuildConnVal());
				vl->append(CWR_vals->Ref());
				ConnectionEvent(connection_CWR_packet, vl);

				Unref(CWR_vals);
			}
		}

	if ( unlikely(flags.ECE()) ) {
			if ( connection_ECE_packet ) {
				RecordVal* ECE_vals = build_ece_packet_val(is_orig, ip, tp);

				val_list* vl = new val_list;
				vl->append(BuildConnVal());
				vl->append(ECE_vals->Ref());
				ConnectionEvent(connection_ECE_packet, vl);
	...
	if (min(caplen, len))
	{
		// Limit two excerpts per flow
		if (excerpts_logged >= MAX_TCP_EXCERPTS) {
			return;
		}

		//if it's part of the initial protocol handshake
		// or if it's low entropy (probably plaintext or protocol identifiers)
		// or if it's mostly ASCII (plaintext)
		// then generate an event
		if ( unlikely ((rel_seq <= 1 && rel_ack <= 1) || (!is_orig && rel_seq <= 1)) || \
		...
		{
			// The below condition is to skip pure ACKS/SYNs that have no payload.
			// Note: we may be interested in the SYNs in the future, but they are 
			// too noisy for now
			/*if ( ( flags.ACK() && ((flags.flags | TH_ACK) == (flags.flags & TH_ACK))) || \
				 ( flags.SYN() && ((flags.flags | TH_SYN) == (flags.flags & TH_SYN))) ) {
				return; 
			}*/
			...
			if (!uncommon_analyzer_attached)
			{
				...
				if ( flags.CWR() )  tcp_flags[tcp_flag_len++] = 'C';
				if ( flags.ECE() )  tcp_flags[tcp_flag_len++] = 'E';
				...
				ConnectionEvent(tcp_backup_event, vl);
				excerpts_logged++;
			}
		}
	}
