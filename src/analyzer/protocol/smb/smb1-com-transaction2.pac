...
type SMB1_transaction2_request(header: SMB_Header) = record {
	...
	parameters : case sub_cmd of {
		...
		TRANS2_SESSION_SETUP          -> session_setup    : trans2_session_setup_request(header);
...
refine connection SMB_Conn += {
        ...
        function proc_trans2_session_setup_request(header: SMB_Header, val: trans2_session_setup_request): bool
        %{
            BifEvent::generate_smb1_trans2_session_setup_request(bro_analyzer(), bro_analyzer()->Conn(), BuildHeaderVal(header));
            return true;
        %}

        function proc_trans2_session_setup_response(header: SMB_Header, val: trans2_session_setup_response): bool
        %{
            return true;
        %}
};

type trans2_session_setup_request(header: SMB_Header) = record {
        # TODO: ...
} &let {
        proc : bool = $context.connection.proc_trans2_session_setup_request(header, this);
};

type trans2_session_setup_response(header: SMB_Header) = record {
        # TODO: implement this.
} &let {
        proc : bool = $context.connection.proc_trans2_session_setup_response(header, this);
};
