...
void Packet::Init(int arg_link_type, pkt_timeval *arg_ts, uint32 arg_caplen,
	...
	time = double(ts.tv_sec) + double(ts.tv_usec) / 1e6;
	...
	if (unlikely( hdr_size < 0))
		{
		Weird("Invalid Link Header Size");
		return;
		}
	if (unlikely( data && cap_len < hdr_size ))
		{
...
void Packet::ProcessLayer2()
	...
	case DLT_EN10MB:
	{
		...
		switch ( protocol )
		{
			...
			case 0x8100:
			case 0x9100:
				if (unlikely( pdata + 4 >= end_of_data ))
				{
					Weird("truncated_link_header");
					return;
				}
			...
			case 0x8864:
				if (unlikely( pdata + 8 >= end_of_data ))
				{
					Weird("truncated_link_header");
					return;
				}
	...
	{
		if (unlikely( pdata + 3 >= end_of_data ))
		{
		...
		if (unlikely( pdata + rtheader_len >= end_of_data ))
		{
			Weird("truncated_radiotap_header");
			return;
		}
		...
		if (unlikely( pdata + len_80211 >= end_of_data ))
		{
			Weird("truncated_radiotap_header");
			return;
		}
	...
	default:
	{
		...
		pdata += GetLinkHeaderSize(link_type);
		if (unlikely( pdata + sizeof(struct ip) >= end_of_data ))
		{
			Weird("truncated_link_header");
			return;
		}
