...
void PktSrc::Process()
	...
	if (unlikely( ! ExtractNextPacketInternal() ))
		return;
...
bool PktSrc::ExtractNextPacketInternal()
{
	...
	if (unlikely( net_is_processing_suspended() && first_timestamp ))
	{
		SetIdle(true);
		return 0;
	}
	...
	if (likely( ExtractNextPacket(&current_packet) ))
	{
		if (unlikely( current_packet.time < 0 ))
		{
			Weird("negative_packet_timestamp", &current_packet);
			return 0;
		}

		if (likely( ! first_timestamp ))
	...
	if (unlikely( pseudo_realtime && using_communication && ! IsOpen() ))
	{
...
bool PktSrc::PrecompileBPFFilter(int index, const std::string& filter)
	...
	if (unlikely( ! code->Compile(BifConst::Pcap::snaplen, LinkType(), filter.c_str(), Netmask(), errbuf, sizeof(errbuf)) ))
		{
