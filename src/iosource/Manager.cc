...
IOSource* Manager::FindSoonest(double* ts)
	...
	if (likely( select(maxx + 1, &fd_read, &fd_write, &fd_except, &timeout) > 0 ))
		{ // Find soonest.
		for ( SourceList::iterator i = sources.begin();
		      i != sources.end(); ++i )
			{
