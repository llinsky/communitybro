...
HashKey* CompositeHash::ComputeHash(const Val* v, int type_check) const
	...
	if ( ! k )
		...
		{
#ifdef LUA_DEBUG
			reporter->Warning("Hash size is zero");
#endif
			return 0;
		}
	...
	{
#ifdef LUA_DEBUG
		reporter->Warning("Hashed val is not a listval");
#endif
		return 0;
	}
	...
	loop_over_list(*tl, i)
		...
		{
#ifdef LUA_DEBUG
			reporter->Warning("Single value hash is zero");
#endif
			return 0;
		}
		}
...
int CompositeHash::ComputeKeySize(const Val* v, int type_check, bool calc_static_size) const
	...
	if ( v )
		...
		{
#ifdef LUA_DEBUG
			reporter->Warning("Hash type tag is not list");
#endif
			return 0;
		}
		...
		{
#ifdef LUA_DEBUG
			reporter->Warning("List length is not equal to true type list length: vl length: %d, tl \
				length: %d", vl->length(), tl->length());
#endif
			return 0;
		}
		}
	...
	loop_over_list(*tl, i)
		...
		{
#ifdef LUA_DEBUG
			reporter->Warning("Hash single key type size is zero");
#endif
			return 0;
		}
		}
