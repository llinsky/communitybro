...
#ifdef ENABLE_LUAJIT
int FuncType::CheckLuaArgs(const type_list* args, bool is_init) const
{
	const type_list* my_args = arg_types->Types();
	...
	for ( int i = 0; i < my_args->length(); ++i )
		if (unlikely( ! same_type((*args)[i], (*my_args)[i], is_init) )) {
			if (likely(BothArithmetic((*args)[i]->Tag(), ((*my_args)[i]->Tag())) ) )
			{
				return 1;
			}
			return 0;
...
void EnumType::CheckAndAddName(const string& module_name, const char* name,
	...
	if ( ! id )
		{
#ifdef LUA_DEBUG
		//reporter->Info("Installing ID in EnumType::CheckAndAddName: module name: %s  enum name:  %s", module_name.c_str(), name);
#endif
