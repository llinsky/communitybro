...
void NetSessions::NextPacket(double t, const Packet* pkt)
	...
	if (unlikely( pkt->hdr_size > pkt->cap_len ))
		{
	...
	if ( pkt->l3_proto == L3_IPV4 )
		{
		if (unlikely( caplen < sizeof(struct ip) ))
			{
		...
		{
		if (unlikely( caplen < sizeof(struct ip6_hdr) ))
			{
			Weird("truncated_IP", pkt);
			return;
			}
...
void NetSessions::DoNextPacket(double t, const Packet* pkt, const IP_Hdr* ip_hdr,
	...
	if (unlikely( pkt->len < len + pkt->hdr_size ))
	{
	...
	if (unlikely( ! ignore_checksums && ip4 &&
	...
	if ( ip_hdr->IsFragment() )
		...
		if (unlikely( caplen < len ))
			...
			if (unlikely( ip_hdr->FragOffset() != 0 ))
				return;
			}
		else
			{
