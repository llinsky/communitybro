...
## Fields of a PSH packet.
##
## .. bro:see:: connection_PSH_packet
type PSH_packet: record {
	is_orig: bool;	##< True if the packet was sent the connection's originator.
	DF: bool;	##< True if the *don't fragment* is set in the IP header.
	ttl: count;	##< The IP header's time-to-live.
	size: count;	##< The size of the packet's payload as specified in the IP header.
	...
	MSS: count;	##< The maximum segment size if present, or 0 if not.
	SACK_OK: bool;	##< True if the *SACK* option is present.
	FIN: bool; ##< True if the FIN flag was also present.
};

## Fields of a URG packet.
##
## .. bro:see:: connection_URG_packet
type URG_packet: record {
	is_orig: bool;	##< True if the packet was sent the connection's originator.
	DF: bool;	##< True if the *don't fragment* is set in the IP header.
	ttl: count;	##< The IP header's time-to-live.
	size: count;	##< The size of the packet's payload as specified in the IP header.
...
## Fields of a CWR packet.
##
## .. bro:see:: connection_CWR_packet
type CWR_packet: record {
	is_orig: bool;	##< True if the packet was sent the connection's originator.
	DF: bool;	##< True if the *don't fragment* is set in the IP header.
	ttl: count;	##< The IP header's time-to-live.
	size: count;	##< The size of the packet's payload as specified in the IP header.
...
## Fields of a ECE packet.
##
## .. bro:see:: connection_ECE_packet
type ECE_packet: record {
	is_orig: bool;	##< True if the packet was sent the connection's originator.
	DF: bool;	##< True if the *don't fragment* is set in the IP header.
	ttl: count;	##< The IP header's time-to-live.
	size: count;	##< The size of the packet's payload as specified in the IP header.
...
export {
  type DCE_RPC::DCE_LSA_STRING: record {
    ...
    max_count:    count   &log &optional;
    offset:       count   &log &optional;
    actual_count: count   &log &optional;
  ...
  type DCE_RPC::DCE_LSA_ACCOUNT_NAME: record {
    ...
    ptr:          DCE_RPC::DCE_LSA_STRING   &log &optional;
  };

  type DCE_RPC::DCE_LSA_PTR_ACCOUNT_NAME: record {
    ref_id:       count                         &log &optional;
    account_name: DCE_RPC::DCE_LSA_ACCOUNT_NAME &log &optional;
  };

  type DCE_RPC::LSA_GETUSERNAME_REQUEST: record {
    system_name:        DCE_RPC::DCE_LSA_STRING           &log &optional;
    ptr_account_name:   DCE_RPC::DCE_LSA_PTR_ACCOUNT_NAME &log &optional;
    ptr_authority_name: string                            &log &optional;
  };
}
...
export {
	...
	## Tunable for sending reporter errors via events, instead of STDERR/syslog
	const report_via_events = F &redef;

	## Tunable global override of stderr logging
	const all_to_stderr = T &redef;
}
...
global SyslogPriority: table[string] of count = {
	["LOG_EMERG"] 	= 0,
	["LOG_ALERT"] 	= 1,
	["LOG_CRIT"] 	= 2, 
	["LOG_ERR"] 	= 3,
...
## Tunable syslog facility for reporter logs
const log_facility = SyslogFacility["LOG_LOCAL5"] &redef;

## Tunable syslog priority for reporter logs
const log_priority = SyslogPriority["LOG_NOTICE"] &redef;

## Tunable identity for reporter logs via syslog
const log_ident: string = "bro" &redef;

## Tunable for sending all reporter messages to syslog.
const reporter_to_syslog = F &redef;
